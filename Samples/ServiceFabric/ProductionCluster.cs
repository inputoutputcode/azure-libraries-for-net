...
{
    public class ProductionCluster
    {
        ...
        public async static void RunSample(IAzure azure)
        {
            ...
            var fabricGatewayProbe = "fabricGatewayProbe";
            var fabricHttpGatewayProbe = "fabricHttpGatewayProbe";
            var httpLoadBalancingRule = "httpRule";
            var fabricGatewayLoadBalancingRule = "fabricGatewayLoadBalancingRule";
            var fabricHttpGatewayLoadBalancingRule = "fabricHttpGatewayLoadBalancingRule";
            ...
            var frontEndNSG = azure.NetworkSecurityGroups.Define(groupName + "nsg")
                    .WithRegion(region)
                    .WithNewResourceGroup(groupName)
                    .DefineRule("Gateway")
                        .AllowInbound()
            ...
            azure.ServiceFabricClusters.Define(clusterName)
                ...
                //.WithAddonFeature(AddOnFeatures.BackupRestoreService)
                //.DefineFabricSettings()
                //    .WithClusterProtectionLevel(ClusterProtectionLevel.EncryptAndSign)
                //.WithClusterCertificate(cert)
                //.WithClientCertificate(cert)
                //.WithProxyCertificate(cert)
                ...
                .AddNodeType(nodeTypeName)
                    ...
                    //.WithDefaultGatewayEndpointPort()
                    //.WithIsPrimary(true)
                    //.WithDurability(DurabilityLevel.Silver)
                    //.WithVmInstanceCount(scaleSet.Capacity)
                //.WithUpgradeDescriptionDefaults()
